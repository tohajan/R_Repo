...
#' 
#' **Note to self**: The plots didn't display in VSCode. Running the code ".vsc.attach()" still didn't fix it. (It worked in an earlier work though)
#' 
#' 
#' ## Bifurcation Diagrams
#' A bifurcation diagram can help visualize how different patterns of growth appear in populations over periods
## ------------------------------------------------------------------------------------------------
minr <- 2.5 # minimum r
maxr <- 4.0 # maximum r
...
#' 
#' 
#' In bifurcation diagrams, the interest is usually focused on later generation because unpredictable patterns generally appear after the population has attained equilibrium.
## ------------------------------------------------------------------------------------------------
ttransients <- 100 # "transient" is a term referring to the early periods (generations) in a population before it reaches equilibrium. In the current example, transient is set to 100, i.e., the diagram should ignore the 1st 100 generations of the population

#' 
## ------------------------------------------------------------------------------------------------
trange <- 400 # the data will contain 400 generations (i.e, after the 1st 100, i.e., it will display only generations 101-500)

#' 
#' 
## ------------------------------------------------------------------------------------------------
#create an empty plot:
par(mfrow = c(1, 1)) # This resets the plot (from a previous code that demarcates the plot for three objects). Note that this is only needed in a basic R environment, not in R Markdown (where codes are organized in chunks, such that restrictions/definitions in a previous plot wouldn't carry over to a next one)

plot(c(minr, maxr), c(0, 1), type = "n", pch = ".", xlab = "r", ylab = "Population density in the last 400 generations")
...
#' 
#' 
#' *Note to self: I wonder why it is not "101:trange" since it's a continuation from ttransients. Same result either way, though.*
#' 
#' Notice the different point cycles (1, 2, 4, etc...) in the resulting bifurcation diagram
#' 
#' 
#' # **DIFFERENTIAL EQUATIONS IN R**
#' Differential equations are used for representing population dynamic when the population changes continuously (unlike at discrete time intervals)
#' 
...
#' 
#' The ode (ordinary differential equation) function used later is contained in the "deSolve" package. Some of the function's arguments are:\
...
#' 
#' ## Continous time logistic equation
#' The above problem can also be solved using the continous time logistic equation (i.e., dN/dT = rN (1 - N/k):
## ------------------------------------------------------------------------------------------------
...
# Note the sigmoidal shape of the curve, which is xteristic of a logistic growth

#' 
#' 
#' ## Lotka-Volterra Predation Equations
#' Instantaneous rate of change in the prey population is given as: dN/dT = (r * N) - a * P * N \
#' Instantaneous rate of change in the predator popn is given as: dP/dT = -(b * P) + f * P * N
## ------------------------------------------------------------------------------------------------
library(lattice)
...
#' 
#' # **STOCHASTIC MODELS**
#' Stochastic models deal with random variables/numbers. Random numbers can be generated by using any of two procedures: \
#' 1. By drawing from existing distributions \
#' 2. By drawing from custom-made selections
#' 
#' ## 1. Using Exisitng Distributions
#' ### A. Uniform Distribution -- the general form of the function is: \
#' runif(n, min, max)
#' 
## ------------------------------------------------------------------------------------------------
runif(15, 1, 10) # generate 15 random numbers btw 1 and 10

#' 
#' If the min and max are not specified, R sets the default to 0 and 1 respectively:
## ------------------------------------------------------------------------------------------------
runif(15) #15 random numbers btw 0 and 1
...
#' This generates the random number of successes of independent trials. The general form of the function is:\
#' rbinom(n, size, prob) \
#' Example: an unbiased coin was tossed 100 times in a row, once a day for a week with a probability that each toss is a success (say, heads)
#' 
## ------------------------------------------------------------------------------------------------
...
## ------------------------------------------------------------------------------------------------
set.seed(3) # this can also be used with the previous functions -- ensures the same set of number is produced each time
...
#' 
#' When the sample specified is larger than the pop, R returns an error because the replace argument is defaulted to "False". To avoid the error message, set replace to "TRUE".
## ------------------------------------------------------------------------------------------------
...
sample(options, 10, replace = TRUE) # there are only two elements ("heads"  and "tails"). But the function asks for 10 drawings. Hence, the replace argument is set to "TRUE."
