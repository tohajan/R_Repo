...
#' 
#' Results: the red genotype seems to have a generally heavier weight than the other two.
#' 
#' Now, let's analyze the differences/relationships. First, try out a silly approach (a simple 1-way ANOVA) that ignores the experiment variable:
## ------------------------------------------------------------------------------------------------
sillyresults <-  lm(weight ~ genotype) 
anova(sillyresults)

#' 
#' Results: p-value=0.2348; we can't reject the NH that genotype has no effect on weight (i.e., there is no evidence that genotype has an effect on weight). But is that actually true?
#' 
#' To answer, try the more appropriate approach -- a 2-way ANOVA that includes the experiment variable. Why would this be helpful? Because it would help account for variability that would otherwise be treated as noise in the 1-way model:
## ------------------------------------------------------------------------------------------------
results <- lm(weight ~ genotype+experiment)
anova(results)

#' 
#' Results: both IVs each have significant effect on weight. Although because this is a default Type I SS approach, these results imply that genotype is being controlled for.
#' 
#' Let's try changing the order in which the IVs are entered into the model:
## ------------------------------------------------------------------------------------------------
results2 <- lm(weight ~ experiment+genotype)
anova(results2)

#' 
#' Results: same as the previous ordering. Same p-values, F-values. This is because the two vars are not collinear with one another [the genotypes are equally distributed within each experiment, i.e., each experiment has exactly one (not less or more) type of the genotypes. So one can't guess the type of experiment by looking just at the genotype, or vice versa]. In short, there is complete ORTHOGONALITY!!! So there is really no need to control for either variable. There is obviously no possibility of confounding! [If there was, one could run the type III Anova]
#' 
...
#' 
#' 
#' Results: Broadly speaking, there is nothing to worry about. Both assumptions appear to check out. The results are not exactly perfect, but again this is a small sample.
#' 
...
#' Results: the actual difference lies in red vs. hairy (p-value=0.023). That is, the primary  source of the variability between genotypes derives from the differences between the red and the hairy genotype.
#' 
...
#' 
#' 
#' Results: The primary differences lie in the following comparisons: 3 vs. 1 (p-value=0.052); 3 vs. 2 (p-value=0.009); and 4 vs. 3 (p-value=0.017).
#' 
#' 
#' --------------------------------------------------------------------------------
#' --------------------------------------------------------------------------------
...
#' 
#' # **Factorial ANOVA** 
#' This takes 2-way ANOVA a step further by considering the effect of interactions between the 2 categorical IVs. ***Interaction*** occurs when the effect of one predictor variable on the response is influenced by another predictor variable in the model. For example, the effect of fertilizer on yield may depend on the pesticide used and vice-versa (i.e., the effect of the pesticide on yield may depend on the fertilizer used). 
#' 
#' In sum, factorial ANOVA helps the researcher to investigate interaction between 2 predictor variables
#'  
#' ## PRACTICE
## ------------------------------------------------------------------------------------------------
...
#' 
#' Results: mean consumption appears slightly higher in male subjects than females. And this observation is the same for both rancid and fresh lards. So there seems to be no interaction going on. That is, preference for fresh or rancid is not dependent on sex. If there was an interaction, the two lines might cross. For example, males might consume higher .amounts of fresh lard than females, but females consume higher amounts of rancid lard than males)
#' 
#' Model a 2-way ANOVA:
## ------------------------------------------------------------------------------------------------
...
#' 
#' Results: as previously shown in the interaction plot, this ANOVA model shows no evidence of interaction between sex and taste (p=0.4503). However, taste alone has a significant effect on the outcome (mean consumption), p=0.0002. Sex has no significant influence (p=0.1460)
#' 
#' ## Model Assumptions
## ------------------------------------------------------------------------------------------------
par(mfrow = c(1,2))
...
#' 
#' Results: there appears to be no significant violation of the two GLM assumptions
#' 
#' Some theoretical questions: \
#' **1.** Would the outcome be different if we changed the order in which the IVs were entered into
#' the model (i.e., taste before sex instead of sex before taste)? \
#' >-- *Answer*: Since the sample is orthogonally distributed between the two IVs (i.e., the male and female subjects are equally distributed between taste type [3 males and 3 females in each of rancid and fresh conditions] - another indication of no interaction), the model results would be the same even after reordering.
#' 
#' **2**. How about using a Type III Sum of Squares approach? \
#' >-- *Answer*: Again, the design is completely orthogonal and the results suggest no evidence of interaction. So, the Type III method would yield same results as Type I above.
#' 
...
#' 
#' Results: As mentioned previously, the results are the same as in Type I SS since there is orthogonality and no evidence of interaction
#' 
#' --------------------------------------------------------------------------------
#' --------------------------------------------------------------------------------
#' --------------------------------------------------------------------------------
...
#' The ANCOVA is a GLM like the linear regression and ANOVA. However, whereas the two have one type of predictor (linear reg--continuous; ANOVA--categorical), ANCOVA has both continuous and categorical predictors. In summary: ANCOVA = continuous response with categorical and continuous predictor(s)
#' 
#' ## Practice 
## ------------------------------------------------------------------------------------------------
...
#' 
#' The data has 30 rows of actual observations and 170 rows of missing observations. Remove them:
## ------------------------------------------------------------------------------------------------
lep <- na.omit(lep) 
dim(lep)
...
#' 
#' Results: bacbef has a significant effect on bacafter but treatmt has no significant effect (NB: the first IV in the model is controlled for according to this 'default' Type I ANOVA method. That IV here bacbef.)
#' 
...
#' 
#' NB: level 1 of treatmt is missing because it is subsumed within the coefficient for the intercept.
#'  
...
#' 
#' 
#' Run the ANOVA using the Type III sum of squares method (see the section on Multiple Regression)
## ------------------------------------------------------------------------------------------------
library(car)
Anova(model1, type = "III")

#' 
#' Results: bacbef has a significant effect on bacafter while controlling for treatment. But treatment has no significant effect while controlling for bacbef. [Recall that according to the Type III approach, when interpreting the effect of one IV, all other IVs in the model are held constant/controlled for]
#' 
...
#' >-- Assumption 2 holds (the residuals tend to follow a normal distribution as seen in the Q-Q plot which is approximately a straight line)
#' 
#' 
#' ## Parallel vs Non-Parallel Slopes
#' ANCOVA assumes approximately the same gradient for the different levels of the categorical var (see the ***parallel slopes*** produced from the regression lines earlier). 
#' 
#' However, it is possible to have a situation where the levels of the categorical var do have different gradient (***non-parallel slopes***). Such a case could be modeled by, for example, assuming an interaction effects between predictors:
## ------------------------------------------------------------------------------------------------
...
#' 
#' Results: \
#' >-- The interaction effect is non-significant, i.e., there is no evidence to reject the NH that the gradients of those relationships do not differ between the different levels of the treatmt var (i.e., no interaction between treatment and bacbef).\
#' >-- There is also no evidence to reject the NH that treatmt has no effect on bacafter score (NB: this is a type I sum of square so we are controlling for bacbef)
...
#' 
#' Again, only bacbef has a significant effect. And there still is no evidence of interaction between the two IVs.
#' 
## ------------------------------------------------------------------------------------------------
model2$coeff
...
#' 
#' As can be seen in the graph, the slopes are no longer parallel. For example, the equation lines for treatments 1 and 2 would cross at a point beyond the y-axis.
#' 
#' 
#' --------------------------------------------------------------------------------
#' --------------------------------------------------------------------------------
...
#' Example Question: "is there an association between winning/not winning status and year?" or "Does the probability of winning vary by year?"
#' 
...
#' Do not reject the NH that there is no association between winning and year (i.e., there is no evidence of an association between winning and year).
#' 
...
#' 
#' Assume there is a population with 1/6 probability of winning a prize (vs. 5/6 of not winning). Can we reject the NH that the data above derives from this population?
#' 
## ------------------------------------------------------------------------------------------------
results <- chisq.test(obsfreq, p = c(1/6,5/6))
results

#' We can reject the NH that the data derives from that population (i.e., there is no evidence to suggest that the data derives from the population).
#' 
#' In what way does the data deviate from said population?
## ------------------------------------------------------------------------------------------------
results$expected

#' As seen, we would expect 104 wins and 520 losses if the data were from the population having a 1/6 winning chance. However, the current data has significantly more wins and fewer losses (136 and 488, respectively) than would be expected.
#' 
#' 
#' ## Example 2
## ------------------------------------------------------------------------------------------------
...
#' 
#' How/what is the deviation (i.e., what would we expect in order to reject the NH)?
## ------------------------------------------------------------------------------------------------
...
#' The warning message arises due to the small sample size
#' Same results as the previous data: No solid evidence to reject the NH hypothesis that the outcome is independent of the cup size.
#' 
## ------------------------------------------------------------------------------------------------
result$expected

#' Due to the relatively low sample size in some cells, the validity of the test may be questioned. One way to address this is by conducting the Fisher's exact test.
#' 
## ------------------------------------------------------------------------------------------------
fisher.test(ofreq)

#' The Fisher's test answers the question, "Considering the observed frequencies, what is the probability of obtaining that outcome or a more extreme one if the NH of there being no association was true?" The response to this question (as seen in the output above) is about 45% (i.e., p=0.4484).
#' 
#' In summary: again, there is no evidence of a relationship between outcome and cup size. So we cannot reject the NH.
